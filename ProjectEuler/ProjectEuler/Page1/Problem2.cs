...
public static partial class Problem2
{
    ...
    private static IEnumerable<IProblemInputSource> GetInputSources()
    {
        yield return new ProblemInputStringSource(
            "sample",
            "Sum of even Fibonacci numbers below 10",
            "10",
            expectedOutput: new Output(10));

        yield return new ProblemInputStringSource(
            "question",
            "Sum of even Fibonacci numbers below 4000000",
            "4000000",
            expectedOutput: new Output(4613732));
    }
    ...
    public record Input(int MaxValue) : IProblemInput, IHasPrintSummary
    {
        public void PrintSummary(IAnsiConsole console, PrintSummaryOptions? options = null)
        {
            ...
            console.MarkupLineInterpolated($"{options.Indent}> Find the sum of all the even-valued terms in the Fibonacci sequence whose values do not exceed [green]{MaxValue}[/].");
        }
    }
    ...
    public class StringInputParser() :
        ProblemInputParser(
            "string",
            "String input",
            "Parses a string in the format: [max=4000000]. E.g. 10, 4000000"),
    ...
    public record Output(int Amount) : IProblemOutput, IHasPrintSummary
    {
        public void PrintSummary(IAnsiConsole console, PrintSummaryOptions? options = null)
        {
            options ??= new PrintSummaryOptions();
